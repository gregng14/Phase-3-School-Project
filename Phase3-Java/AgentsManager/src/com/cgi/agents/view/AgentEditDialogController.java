...
import com.cgi.agents.model.AgencyDB;
 ...
 * @author 723243
 */
public class AgentEditDialogController {
    ...
    public void setAgent(Agent agent) {
        ...
        if (agent != null && agent.getAgentId() > 0) {
            ...
            //cboAgency.getSelectionModel().select();
...
//        chkActive.setSelected(true);
    ...
    private boolean isInputValid() {
        ...
        if ((tfFirstName.getText().length() == 0) || (tfFirstName.getText().length() > 20)) {
            errorMessage += "First name has to be between 1 and 20 characters.\n";
        }
        
        if (tfMidInitial.getText().length() > 5) {
            errorMessage += "Middle initial cannot be more than 5 characters.\n";
        }
        
        if ((tfLastName.getText().length() == 0) || (tfLastName.getText().length() > 20)) {
            errorMessage += "Last name has to be between 1 and 20 characters.\n";
        }
        
        phoneNumMatcher = phoneNumPattern.matcher(tfPhone.getText());
        
        if ((tfPhone.getText().length() == 0) || (tfPhone.getText().length() > 20)) {
        ...
        if ((tfEmail.getText().length() == 0) || (tfEmail.getText().length() > 50)) {
        ...
        if (chkActive.isSelected() && ((tfPosition.getText().length() == 0) || (tfPosition.getText().length() > 20))) {
